...
def make_stop_div_icon(route_color: str, stop_text: str, size_scale: float = 1.0):
    base_size = 28
...
def make_assigned_square_icon(route_color: str, stop_text: str, size_scale: float = 1.0):
    base_size = 30
    icon_size = max(16, int(round(base_size * size_scale)))
    border_size = max(1, int(round(2 * size_scale)))
    ...
    for route in route_list:
        ...
        for _, row in route_grouped.iterrows():
            ...
            if is_assigned_pin:
                ...
                icon_obj = make_assigned_square_icon(pin_color, pin_text, size_scale=size_scale)
            else:
                pin_text = str(row.get("pin_label", ""))
                icon_obj = make_stop_div_icon(pin_color, pin_text, size_scale=size_scale)
