...
def build_group_summary_df(group_assignment_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for gname in group_names:
        ...
        route_mark = "●" if spread_km <= 10 else "◐" if spread_km <= 18 else "○"

        stop_gap = abs(total_stops - avg_stops) if avg_stops else 0
        stop_mark = "●" if stop_gap <= 5 else "◐" if stop_gap <= 10 else "○"

        box_mark = "●" if box_deviation_rate <= 0.2 else "◐" if box_deviation_rate <= 0.35 else "○"

        time_mark = "●" if total_minutes <= 240 else "◐" if total_minutes <= 270 else "○"

        reason_text = f"라우트 {route_mark} / 스톱수 {stop_mark} / 수량차이 {box_mark} / 예상시간 {time_mark}"
