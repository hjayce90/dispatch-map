...
class MemoMatchType(models.TextChoices):
    COMPANY_ID = "company_id", "Company ID"
    ADDRESS_NORM = "address_norm", "Address Norm"
...
class CustomerMemo(TimeStampedModel):
    match_type = models.CharField(max_length=20, choices=MemoMatchType.choices)
    ...
    address_norm = models.CharField(max_length=500, blank=True, db_index=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["match_type", "match_key"],
                name="dispatch_customer_memo_unique_match",
            )
        ]

    def __str__(self):
        return f"{self.match_type}:{self.match_key}"
